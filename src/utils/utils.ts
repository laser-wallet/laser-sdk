...
export function encodeExec(transaction: Transaction): string {
    return encodeFunctionData(walletAbi, "exec", [
        transaction.to,
        transaction.value,
...
///@dev Checks if the wallet has enough ETH to pay for the transaction gas.
// export async function canWalletPayFees(tx: Transaction, walletBalance: BigNumber): Promise<any> {
//     const ethToPay = baseFee.add(maxPriorityFeePerGas).mul(gas);

//     // Infura's fee.
//     const ethToPayWithFee = ethToPay.div(100).mul(6);

//     return walletBalance.gt(ethToPayWithFee);
// }
